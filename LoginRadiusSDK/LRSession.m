...
- (nullable NSString*) accessToken {
    ...
    if([[LoginRadiusSDK sharedInstance] useKeychain])
    {
        ...
        LRKeychainWrapper *keychain = [[LRKeychainWrapper alloc] initWithIdentifier:[LoginRadiusSDK siteName] accessGroup:groupKey];
        
        
        NSData *tokenString = [keychain dataForKey:@"lrAccessToken"];
        
        token = [[NSString alloc] initWithData:tokenString encoding:NSUTF8StringEncoding];
...
- (instancetype)initWithAccessToken:(NSString*_Nonnull)token userProfile:(NSDictionary*_Nonnull)userProfile{
    ...
    if([[LoginRadiusSDK sharedInstance] useKeychain])
    {
        ...
        [keychain setData:tokenData forKey:@"lrAccessToken"];
